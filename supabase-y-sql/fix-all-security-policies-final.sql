-- SCRIPT FINAL PARA LIMPIAR TODAS LAS POLÍTICAS RLS (CORREGIDO)
...
DROP POLICY IF EXISTS "Allow users to manage messages in their matches" ON public.messages;
...
-- MESSAGES (CORREGIDO - usando match_id en lugar de sender_id/recipient_id)
...
CREATE POLICY "messages_delete_policy" ON public.messages FOR DELETE TO authenticated USING (
    match_id IN (
        SELECT matches.id 
        FROM matches 
        WHERE matches.user1_id = auth.uid() OR matches.user2_id = auth.uid()

-- ✨ RECREACIÓN LIMPIA Y OPTIMIZADA - AGARCH-AR
...
-- Índices optimizados
CREATE INDEX idx_profiles_ubicacion ON profiles (ubicacion_lat, ubicacion_lng) WHERE ubicacion_lat IS NOT NULL AND ubicacion_lng IS NOT NULL;
CREATE INDEX idx_profiles_genero ON profiles (genero);
CREATE INDEX idx_profiles_ultimo_acceso ON profiles (ultimo_acceso DESC);
...
CREATE INDEX idx_posts_ubicacion ON posts (ubicacion_lat, ubicacion_lng) WHERE ubicacion_lat IS NOT NULL AND ubicacion_lng IS NOT NULL;
...
-- Índices optimizados
CREATE INDEX idx_messages_participants ON messages (sender_id, recipient_id, created_at DESC);
CREATE INDEX idx_messages_recipient ON messages (recipient_id, es_leido, created_at DESC);
...
-- Índices optimizados
CREATE INDEX idx_comentarios_publicacion ON comentarios (publicacion_id, creado_en DESC);
...
CREATE INDEX idx_user_likes_liked_user ON user_likes (liked_user_id);
...
-- Índices optimizados
CREATE INDEX idx_notifications_user ON notifications (user_id, es_leida, created_at DESC);
...
-- Índices optimizados
CREATE INDEX idx_reports_estado ON reports (estado, created_at DESC);
...
SELECT 'TABLAS CREADAS EXITOSAMENTE - 11 TABLAS ESENCIALES' as resultado;

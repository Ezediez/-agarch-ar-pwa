-- ROLLBACK CORREGIDO: SIN TABLA COMMENTS
-- Crear solo políticas para tablas que existen
...
-- 2. CREAR POLÍTICAS ESENCIALES SOLO PARA TABLAS EXISTENTES
...
-- COMENTARIOS (tabla que sí existe): Ver públicos, escribir propios
...
SELECT 'POLITICAS DESPUÉS DEL ROLLBACK CORREGIDO' as resultado;
SELECT 
    tablename,
...
-- 5. CONTAR TOTAL DE POLÍTICAS
SELECT 'TOTAL POLITICAS ACTIVAS' as resultado, COUNT(*) as cantidad
FROM pg_policies 
WHERE schemaname = 'public';

SELECT 'ROLLBACK CORREGIDO COMPLETADO' as estado;
SELECT 'Deberían quedar aproximadamente 20-22 políticas simples' as expectativa;

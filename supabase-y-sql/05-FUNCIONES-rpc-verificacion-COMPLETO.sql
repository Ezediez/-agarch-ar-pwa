...
-- =====================================================
-- RESTAURAR DATOS DESDE BACKUP (SIN ERRORES)
-- =====================================================

-- Restaurar profiles (solo los válidos)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'backup_profiles') THEN
        ...
        -- Actualizar columnas adicionales de forma segura
        BEGIN
            UPDATE profiles SET email = bp.email FROM backup_profiles bp WHERE profiles.id = bp.id AND bp.email IS NOT NULL;
        EXCEPTION WHEN OTHERS THEN NULL; END;
        
        BEGIN
            UPDATE profiles SET bio = bp.bio FROM backup_profiles bp WHERE profiles.id = bp.id AND bp.bio IS NOT NULL;
        EXCEPTION WHEN OTHERS THEN NULL; END;
        
        BEGIN
            UPDATE profiles SET genero = bp.genero FROM backup_profiles bp WHERE profiles.id = bp.id AND bp.genero IS NOT NULL;
        EXCEPTION WHEN OTHERS THEN 
            BEGIN
                UPDATE profiles SET genero = bp.gender FROM backup_profiles bp WHERE profiles.id = bp.id AND bp.gender IS NOT NULL;
            EXCEPTION WHEN OTHERS THEN NULL; END;
        END;
        
        BEGIN
            UPDATE profiles SET profile_picture_url = bp.profile_picture_url FROM backup_profiles bp WHERE profiles.id = bp.id AND bp.profile_picture_url IS NOT NULL;
        EXCEPTION WHEN OTHERS THEN NULL; END;
        
        BEGIN
            UPDATE profiles SET is_verified = COALESCE(bp.is_verified, false) FROM backup_profiles bp WHERE profiles.id = bp.id;
        EXCEPTION WHEN OTHERS THEN NULL; END;
        
        BEGIN
            UPDATE profiles SET is_vip = COALESCE(bp.is_vip, false) FROM backup_profiles bp WHERE profiles.id = bp.id;
        EXCEPTION WHEN OTHERS THEN NULL; END;
    END IF;
END $$;

-- Restaurar posts (solo los que tienen contenido válido)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'backup_posts') THEN
        -- Insertar posts que tienen al menos algún contenido
        INSERT INTO posts (id, user_id, texto, imagen_url, video_url, created_at, updated_at)
        SELECT 
            bp.id, 
            bp.user_id,
            COALESCE(bp.texto, bp.text, 'Post restaurado') as texto, -- Garantizar contenido
            bp.imagen_url,
            bp.video_url,
            COALESCE(bp.created_at, NOW()) as created_at,
            COALESCE(bp.updated_at, NOW()) as updated_at
        ...
        AND (
            (bp.texto IS NOT NULL AND bp.texto != '') OR
            (bp.text IS NOT NULL AND bp.text != '') OR
            (bp.imagen_url IS NOT NULL AND bp.imagen_url != '') OR
            (bp.image_url IS NOT NULL AND bp.image_url != '') OR
            (bp.video_url IS NOT NULL AND bp.video_url != '')
        )
        ON CONFLICT (id) DO NOTHING;
        
        -- Si no hay posts con contenido, crear al menos uno de ejemplo
        IF NOT EXISTS (SELECT 1 FROM posts) THEN
            INSERT INTO posts (user_id, texto, created_at, updated_at)
            SELECT 
                p.id,
                'Bienvenido a AGARCH-AR! 🎉',
                NOW(),
                NOW()
            FROM profiles p
            LIMIT 1
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'backup_messages') THEN
        ...
        AND bm.sender_id != bm.recipient_id
        ON CONFLICT (id) DO NOTHING;
    END IF;
EXCEPTION
    WHEN OTHERS THEN NULL;
...
-- Restaurar otras tablas de forma segura
DO $$
BEGIN
    -- Comentarios
    BEGIN
...
-- =====================================================
-- CREAR PERFILES PARA USUARIOS AUTH SIN PERFIL
-- =====================================================
...
SELECT 'FUNCIONES RPC CREADAS' as diagnostico;
SELECT 
...
SELECT 'POLÍTICAS RLS ACTIVAS' as diagnostico;
SELECT 
    tablename,
...
SELECT 'STORAGE BUCKETS' as diagnostico;
SELECT 
    id as bucket_name,
    public as publico,

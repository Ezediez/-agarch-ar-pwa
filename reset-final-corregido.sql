-- SCRIPT FINAL CORREGIDO PARA RESETEAR SUPABASE
-- Ejecutar en el Editor SQL de Supabase

-- =====================================================
-- PASO 1: LIMPIAR FUNCIONES PROBLEMÁTICAS
-- =====================================================
...
-- =====================================================
-- PASO 4: CREAR FUNCIONES SEGURAS (CORREGIDAS)
-- =====================================================
...
-- Función para búsqueda de usuarios (SIN PARÁMETROS DUPLICADOS)
...
BEGIN
    ...
    WHERE p.id != user_id
        ...
        AND (looking_for_filter IS NULL OR p.gender = ANY(looking_for_filter))
    ORDER BY distance;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, auth;
...
CREATE POLICY "posts_policy" ON public.posts FOR ALL TO authenticated USING (true) WITH CHECK (auth.uid() = id);
CREATE POLICY "likes_policy" ON public.likes FOR ALL TO authenticated USING (true) WITH CHECK (auth.uid() = id);
CREATE POLICY "comentarios_policy" ON public.comentarios FOR ALL TO authenticated USING (true) WITH CHECK (auth.uid() = id);
CREATE POLICY "stories_policy" ON public.stories FOR ALL TO authenticated USING (true) WITH CHECK (auth.uid() = id);
CREATE POLICY "matches_policy" ON public.matches FOR ALL TO authenticated USING (auth.uid() = user_id_1 OR auth.uid() = user_id_2) WITH CHECK (auth.uid() = user_id_1 OR auth.uid() = user_id_2);
CREATE POLICY "messages_policy" ON public.messages FOR ALL TO authenticated USING (auth.uid() = sender_id OR auth.uid() = receiver_id) WITH CHECK (auth.uid() = sender_id);
CREATE POLICY "notifications_policy" ON public.notifications FOR ALL TO authenticated USING (auth.uid() = id) WITH CHECK (auth.uid() = id);
...
CREATE INDEX IF NOT EXISTS idx_posts_user_id ON public.posts(id);
CREATE INDEX IF NOT EXISTS idx_posts_created_at ON public.posts(created_at);
CREATE INDEX IF NOT EXISTS idx_likes_post_id ON public.likes(post_id);
CREATE INDEX IF NOT EXISTS idx_likes_user_id ON public.likes(id);
CREATE INDEX IF NOT EXISTS idx_comentarios_post_id ON public.comentarios(post_id);
CREATE INDEX IF NOT EXISTS idx_comentarios_user_id ON public.comentarios(id);
CREATE INDEX IF NOT EXISTS idx_stories_user_id ON public.stories(id);
...
CREATE INDEX IF NOT EXISTS idx_notifications_user_id ON public.notifications(id);

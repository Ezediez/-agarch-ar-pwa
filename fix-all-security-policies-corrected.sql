-- SCRIPT AGRESIVO PARA LIMPIAR TODAS LAS POLÍTICAS RLS (CORREGIDO)
...
-- MESSAGES (CORREGIDO - usando sender_id y recipient_id en lugar de receiver_id)
CREATE POLICY "messages_insert_policy" ON public.messages FOR INSERT TO authenticated WITH CHECK (auth.uid() IN (sender_id, recipient_id));
CREATE POLICY "messages_select_policy" ON public.messages FOR SELECT TO authenticated USING (auth.uid() IN (sender_id, recipient_id));
